...
// @version        1.1
// @author         xhhjin
// @description    方便的在各个网页搜索引擎之间跳转,改自Ted423的脚本
// @namespace      https://coding.net/u/xhhjin/p/FirefoxConfiguration/git/blob/master/
// @updateURL      https://coding.net/u/xhhjin/p/FirefoxConfiguration/git/blob/master/SearchJump.user.js
// @downloadURL    https://coding.net/u/xhhjin/p/FirefoxConfiguration/git/blob/master/SearchJump.user.js
// @include        *
// @match          *://*/*
// @grant          none
// @run-at         document-end
...
{name: "google",
enabled: true,
url: /^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}(\/)?(?!.*preferences\?)(?!.*cse\/)(?!.*tbm=isch)(?!.*tbs=sbi)/,
engineList: 'web',
style: "",
insertIntoDoc: {
target: 'css;#gb',
...
/*----------------------------------------------------------------*/
{name: "baidu网页搜索",
url: /^https?:\/\/(www|news)\.baidu\.com(?!.*\/.*\/)(?!.*error\.html)(?!.*forbidden\.html)/i,
enabled: true,
engineList: "web",
style: "",
insertIntoDoc: {
...
keyword: function () {
var input;
if(document.getElementById('kw')&&document.getElementById('kw').value!="") 
input = document.getElementById('kw'); 
else input = document.getElementById('kw1'); 
if (input) return input.value;
},
where: 'afterEnd',
},
...
etc:function(){document.getElementsByClassName("header")[0].style.height='70px';}
},
{name:"好搜",
url:/^https?:\/\/(www\.haosou\.com|so\.360\.cn)\/s/i,
engineList:"web",
enabled:true,
style:"",
insertIntoDoc:{
...
{name:"AOL网页搜索",
url:/^https?:\/\/www\.aolsearch\.com\/search/i,
engineList:"web",
enabled:true,
style:"",
insertIntoDoc:{
keyword:'css;#csbquery1',
target:'css;#inner',
where:'afterEnd'
}
},
...
/*如何添加搜索引擎:*/
/*比如:*/
/*list.web[0]=['Google','http://www.google.com/search?q=%s&ie=utf-8&oe=utf-8','http://www.google.cn/favicon.ico','gbk']*/
/*list.web[排序(从小到大排列,小于0不显示相应引擎)]['显示在网页上的名字','关键字变量用%s代替','站点的图标(可以没有)',是否不接受UTF-8编码(不接受的话..填相应的编码..目前支持gbk或gb2312)]*/
/*网页搜索列表*/
list.web=[];
list.web[0]=['Google','https://www.google.com.hk/search?q=%s&ie=utf-8&oe=utf-8',icons.google]
list.web[1]=['百度','http://www.baidu.com/s?wd=%s&ie=utf-8',icons.baidu]
list.web[2]=['Bing','http://cn.bing.com/search?q=%s&pc=OPER',icons.bing]
list.web[3]=['好搜','http://www.haosou.com/s?ie=utf-8&q=%s',icons.haosou]
//list.web[4]=['有道','http://www.youdao.com/search?q=%s&ue=utf8',icons.youdao]
//list.web[5]=['雅虎','http://www.yahoo.cn/s?q=%s',icons.yahoo]
//list.web[6]=['AOL','http://www.aolsearch.com/search?q=%s',icons.aol]
...
if(siteInfos_i.url.test(url)){/*域名匹配*/
matchedRule=siteInfos_i;
if(siteInfos_i.etc!=undefined)siteInfos_i.etc();
break;
};
};
...
function getElement(selsector){/*对插入显示的定位相关判断*/
var ret;
...
/*容器.*/
var container=document.createElement('span');/*向页面body末尾加入style（全局样式）*/
/*container.style.cssText=matchedStyle;*/
container.style.cssText='position: fixed; bottom:0; right:0; z-index:1; margin-top: 10px; margin-bottom: 2px';;
...
if(expand && matchedRule.url.test(array_i[1])){/*加载搜索引擎的检测*/
continue;
};
...
tempArray.push('<a'+href+'class="sej-engine"'+address+target+encoding+'>'+img+array_i[0]+'</a>');
};
};
...
engineListInnerHTML=getEngineListInnerHTML(details_i[2],undefined);
...
DropDownListObj.zIndex=99999999;
...
ddStyle.left= aRect.left + windowScrollXY.x +'px';
};
...
this.ddWidth=parseInt(width,0);
this.dropDown.style.display='none';
};
},
init:function(){
var dropDown=document.createElement('span');/*下拉列表*/
...
mouseEventListener.add('mouseenter', a, mEnterHandler);
mouseEventListener.add('mouseenter', dropDown,mEnterHandler);
...
mouseEventListener.add('mouseleave', a, mLeaveHandler);
mouseEventListener.add('mouseleave', dropDown, mLeaveHandler);
...
function insertIntoDocument(ele,target,where){//加入到页面之中
if(!where){
if(prefs.debug)
alert("InsertIntoDoc的 where 项,不存在");
return;
};
